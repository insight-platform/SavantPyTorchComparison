...
def resize_preserving_aspect(
    ...
    :param target_size: Target size (img_h, img_w).
    ...
    if (img_h == target_h and img_w < target_w) or (
        ...
        return pad_to_aspect(img, target_size, pad_color)
    ...
    if math.isclose(img_aspect, target_aspect, rel_tol=1e-3):
        ...
        new_img = np.full((target_h, target_w, img_c), pad_color, dtype=np.uint8)
